...
/// Le portage Swift doit produire exactement les mêmes métadonnées que la
/// version web. Ces tests le vérifient contre un CSV réellement engendré par
/// le bouton d'export, sur les mêmes données : c'est la seule façon de
/// garantir qu'un scan taggé par l'une ou l'autre application soit identique.
final class ExifExportTests: XCTestCase {

    private func loadBackup() throws -> Backup {
        let url = try XCTUnwrap(
            Bundle.module.url(forResource: "backup-web", withExtension: "json",
    ...
    private func loadReferenceCSV() throws -> String {
        let url = try XCTUnwrap(
            Bundle.module.url(forResource: "export-web", withExtension: "csv",
                              subdirectory: "Fixtures"))
    ...
    /// Le test qui compte : la sortie Swift et la sortie web doivent coïncider.
    func testMatchesTheWebExportExactly() throws {
        ...
        for index in 0..<min(producedLines.count, referenceLines.count) {
            XCTAssertEqual(
                ...
                "ligne \(index) divergente entre Swift et web")
        }
    }
